...
int main(int argc, char ** argv){
  ...
  while(ch != EOF)
    ...
    if(row > 9)
      {
      fprintf(stderr,"Usage: Higher number of rows than required\n");
      return EXIT_FAILURE;                          
      }
    ...
    if(col==10)
      ...
      if(ch != '\n')
	{
	fprintf(stderr,"Usage: Missing new line at the end of row\n");
	return EXIT_FAILURE;                          
	}
      }
    else
      ...
      if(ch == '\n')
	{
        fprintf(stderr,"Usage: Less columns than required\n");
        return EXIT_FAILURE;                          
	}
  ...
  if(row<10)
    {
    fprintf(stderr,"Usage: Less rows than required\n");
    return EXIT_FAILURE;                          
    }
