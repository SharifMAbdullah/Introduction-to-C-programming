...
char value_letter(card_t c) {
  if(c.value>=2 && c.value<10) return '0'+c.value;
  else
    {
      switch(c.value)
	{
...
card_t card_from_letters(char value_let, char suit_let) {
  ...
  if(value_let> '0' && value_let<='9') temp.value = value_let - '0';
  else
    {
      switch(value_let)
	{
...
card_t card_from_num(unsigned c) {
  ...
  if(cardsValue>=2) temp.value = cardsValue;
  else temp.value = cardsValue + 13;
    return temp;
}
